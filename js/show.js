const filterItem = document.querySelector('.page8__choices');
const filterImage = document.querySelectorAll('.page8__img');

window.onload = () => { // once window loaded 
   filterItem.onclick = (selectedItem) => { // when user clicked on filterItem
      if(selectedItem.target.classList.contains('page8__choice')) { // if user click element has class .item
